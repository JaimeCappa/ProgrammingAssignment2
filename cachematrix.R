...
makeCacheMatrix <- function(x = matrix()) { ## create a matrix object x and some associated sub-functions/methods
  
      m <- NULL ## define the cache m
      set <- function (y) {
            x <<- y ## assign the input matrix y to the variable x in the parent environment
            m <<- NULL ## re-initialize m in the arent environment to null
      }
...
cacheSolve <- function(x, ...) {  ## return a matrix that is the inverse of 'x'
      ...
      m <- x$getInverse () ## gets inverse of a matrix using makeCacheMatrix function above
      if (!is.null (m)) { ##checks to see if the inverse has already been calculated
            message ("getting cached data")
            return (m) ## if it has, it skips computation and returns m
      }
  
      data <- x$get () ## gets the matrix
      m <- inverse (data, ...) ## calculates the matrix inverse 
      x$setInverse (m) ##sets the value of the inverse in the cache using setInverse function
      m ## returns m
}
